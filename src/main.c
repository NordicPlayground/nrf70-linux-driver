			   ...
			   struct device *dev)
{
	...
	status = nrf_wifi_fmac_fw_load(rpu_ctx_lnx->rpu_ctx, &fw_info);
...
struct nrf_wifi_ctx_lnx *nrf_wifi_fmac_dev_add_lnx(void)
{
	...
	rpu_ctx = nrf_wifi_fmac_dev_add(rpu_drv_priv.fmac_priv, rpu_ctx_lnx);
	...
	status = nrf_wifi_wlan_fmac_fw_load(rpu_ctx_lnx, dev);
...
nrf_wifi_fmac_dev_init_lnx(struct nrf_wifi_ctx_lnx *rpu_ctx_lnx)
{
	...
	status = nrf_wifi_fmac_dev_init(rpu_ctx_lnx->rpu_ctx, NULL,
					...
					&tx_pwr_ceil_params);
#else
	status = nrf_wifi_fmac_dev_init_rt(rpu_ctx_lnx->rpu_ctx,
#ifdef CONFIG_NRF_WIFI_LOW_POWER
					   sleep_type,
#endif /* CONFIG_NRF_WIFI_LOW_POWER */
					   NRF_WIFI_DEF_PHY_CALIB, op_band,
...
void nrf_wifi_disp_scan_res_callbk_fn(
	void *os_vif_ctx,
...
#ifndef CONFIG_NRF700X_RADIO_TEST
void nrf_wifi_cookie_rsp_callbk_fn(
...
void nrf_wifi_twt_teardown_callbk_fn(
	void *os_vif_ctx,
...
#ifdef CONFIG_NRF_WIFI_LOW_POWER
void nrf_wifi_twt_sleep_callbk_fn(
...
int __init nrf_wifi_init_lnx(void)
{
	...
	callbk_fns.unprot_mlme_mgmt_rx_callbk_fn =
		&nrf_wifi_cfg80211_unprot_mlme_mgmt_rx_callbk_fn;
...
#else
	nrf_wifi_fmac_deinit_rt(rpu_drv_priv.fmac_priv);
#endif /* !CONFIG_NRF700X_RADIO_TEST */
